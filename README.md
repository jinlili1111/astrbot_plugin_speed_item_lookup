...
- 本地 `data/item_names.json` 未收录时，可兜底查询 MySQL `player.itemallnew`。
- 支持限制允许群。
- 支持图片缺失时仍回复文字信息。
...
| `mysql_fallback_enabled` | `false` | 本地未收录时是否启用 MySQL 兜底查询 |
| `mysql_host` | `""` | MySQL 主机地址；容器访问宿主机通常填 Docker 网关 IP |
| `mysql_port` | `3306` | MySQL 端口 |
| `mysql_user` | `""` | MySQL 只读用户名 |
| `mysql_password` | `""` | MySQL 只读用户密码 |
| `mysql_database` | `player` | MySQL 数据库名 |
| `mysql_table` | `itemallnew` | MySQL 物品表名 |
| `mysql_id_column` | `ID` | MySQL 物品 ID 字段 |
| `mysql_name_column` | `Name` | MySQL 物品名称字段 |
| `mysql_type_column` | `Type` | MySQL 物品 Type 字段 |
| `mysql_mess_column` | `Mess` | MySQL 物品类型说明字段 |
| `mysql_timeout_sec` | `3` | MySQL 连接与查询超时秒数 |
