...
AstrBot 插件：玩家发送 `/itemid 物品ID` 或 `/itemid 名称` 时，机器人回复 QQ 飞车物品名称、类型和 `https://iips.speed.qq.com/images/<ID>.png` 对应图片。

## 功能

- 使用 `/itemid` 指令触发，不需要 @ 机器人。
- 支持物品 ID 精确查询，所有类型都会回复，不再限制赛车/皮肤。
- 支持名称模糊搜索。
- 多个匹配时返回候选列表，并支持第二轮 `/itemid 序号` 筛选。
- 支持限制允许群。
- 支持图片缺失时仍回复文字信息。
